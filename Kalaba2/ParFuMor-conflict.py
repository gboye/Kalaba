...
class Forme:
    ...
    '''
    def __init__(self,sigma,forme,glose,decoupe,detoure,lexRacine):
        self.sigma=sigma
        self.forme=forme
        self.glose=glose
    ...
    def __repr__(self):
        return u"%s:\t%s\t%s\t%s\t%s\tracine: %s"%(self.sigma,self.forme, self.glose, self.decoupe, self.detoure, self.racine)
...
#        print "initTableau",classe, stem, nom
        ...
        for case in paradigmes.getSigmas(classes):
            ...
            flexion=Forme(case,forme,glose,decoupe,radical,lexRacine)
...
class Lexeme:
    '''
    s fléchies d'un lexème suivant sa classe flexionnelle
    '''
